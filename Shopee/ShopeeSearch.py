...
def data_to_csv( keyword, data):
    ...
    attributes_dict={'shopid':'賣家ID','shopacc':'賣家帳號','shopname':'賣家名稱', 'itemid':'商品ID', 'name':'商品名稱', 'shop_location':'賣場位置', 'ctime':'上架時間', 'historical_sold':'已售出數量', 'stock':'庫存', 'liked_count':'喜歡數', 'view_count':'瀏覽數', 'price':'實際價格'}
        ...
        for i in range( 0, len(data_item), 1):
            ...
            for j in range( 0, len(attributes_key), 1):
                ...
                if key == 'name':
                    ...
                    table_dict[attributes_dict['shopacc']] = shop_account
                    table_dict[attributes_dict['shopname']] = shop_name
                    continue
                elif key == 'shopname':
                    continue
                elif key == 'ctime':
...
# keyword = '三多葉黃素'
# data = commodity_search(keyword)
# if data != None:
#     data_to_csv( keyword, data)
